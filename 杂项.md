...
`get_child`：获取当前节点下的所有子节点（包括属性、注释、数据）。如果路径不存在，直接抛出异常

`get_child_optional`：获取子节点。如果路径不存在，不抛出异常
...
可以通过 get<>(属性) 获取当前节点的属性值；对于注释节点，使用 data() 来获取注释内容

可以通过 get_child() 获取某个节点下的所有子节点或者当前节点下的所有子节点
...
int main() 
{
    ...
    for (BOOST_AUTO(pos, child.begin()); pos != child.end(); ++pos)
    {
        ...
        if ("<xmlattr>" == pos->first)
        {
            ...
            // 获取noexits属性的值，如果没有则用This is default替代
